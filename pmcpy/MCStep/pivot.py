    ...
    def mc_move(self) -> bool:
        ...
        if self.rotate_end:
            ...
            if self.numba_tail_rotation:
                ...
                self.chain.conf[id+1:,:3,:3] = G @ self.chain.conf[id+1:,:3,:3]
                vecs = self.chain.conf[id+1:,:3,3] - self.chain.conf[id:-1,:3,3]
                rotated_vecs = (G @ vecs.T).T
                for i in range(0,self.nbp-id-1):
                    self.chain.conf[id+1+i,:3,3] = self.chain.conf[id+i,:3,3] + rotated_vecs[i]
        ...
        # rotate front    
            ...
            if self.numba_tail_rotation:
                ...
                rotated_vecs = (G @ vecs.T).T
                for i in range(0,id):
                    self.chain.conf[id-1-i,:3,3] = self.chain.conf[id-i,:3,3] - rotated_vecs[id-1-i]

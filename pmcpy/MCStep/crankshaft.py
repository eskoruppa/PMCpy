...
class Crankshaft(MCStep):
    ...
    def __init__(
        ...
        if range_id1 is not None or range_id2 is not None:
            ...
            if self.range_id2 < self.range_id1:
                self.range_id2 = self.range_id1 + (self.range_id2-self.range_id1)%self.nbp
    ...
    def mc_move(self) -> bool:
        ...
        TBn_rot[:3,3] = self.chain.conf[idB,:3,3] - Rlab @ (self.chain.conf[idB,:3,3] - self.chain.conf[idBn,:3,3])
        ...
        if self.numba_tail_rotation:
            ...
            # rotate triads
            self.chain.conf[np.arange(idA+1,idA+hingedist-1)%self.nbp,:3,:3] = Rlab @ self.chain.conf[np.arange(idA+1,idA+hingedist-1)%self.nbp,:3,:3]
            # move positions
            vecs = self.chain.conf[np.arange(idA+1,idA+hingedist-1)%self.nbp,:3,3] - self.chain.conf[np.arange(idA,idA+hingedist-2)%self.nbp,:3,3]
            rotated_vecs = (Rlab @ vecs.T).T
            id = idA%self.nbp
            for i in range(hingedist-2):            
                idm1 = id
                id   = (id+1)%self.nbp
                self.chain.conf[id,:3,3] = self.chain.conf[idm1,:3,3] + rotated_vecs[i]

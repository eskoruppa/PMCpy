...
class MidstepMove(MCStep):
    ...
    def __init__(
        ...
        for i in range(len(midstep_triad_ids)):
            if i < len(midstep_triad_ids)-1 and self.midstep_triad_ids[i+1]-self.midstep_triad_ids[i] <= 1:
                raise ValueError(f'MidstepMove: Current restrained midstep triads need to be at least 2 triads apart.')
            if not self.closed and self.midstep_triad_ids[i] < 0 or self.midstep_triad_ids[i] > self.nbps:
                raise ValueError(f'MidstepMove: Midstep triad assignment out of bounds ({self.midstep_triad_ids[i]})')
            if not self.closed and self.midstep_triad_ids[i] == self.nbps:
                raise ValueError(f'MidstepMove: Cannot assign midstep triad fix to last triad for open configuration. (nbp: {self.nbp}, triad assignment: {self.midstep_triad_ids[i]})')
